...
export async function POST({ request, platform }) {
	...
	if (!(formData.has('source') && formData.has('target')))
		error(400, 'webmentions must specify a source and target');
	...
	if (targetUrl.hostname !== PUBLIC_HOSTNAME)
		error(400, `that doesn't look like a webmention for ${PUBLIC_HOSTNAME}`);
	...
	return await fetch(sourceUrl, {
		...
		async (response) => {
			...
			const validItem = mf2Items.find((mfItem) => {
				return (
					Object.entries(mfItem.properties).some(([propName, property]) => {
						return (
							Object.values(VALID_WEBMENTION_TARGET_TYPE).includes(
								propName as VALID_WEBMENTION_TARGET_TYPE
							) && property.map((prop) => prop.toString()).some(matchesTarget)
						);
					}) &&
					(mfItem.properties.url == null ||
						mfItem.properties.url?.some((url: string) => url == sourceUrl.toString()) ||
						(TRUSTED_THIRD_PARTY_MICROFORMATS_SOURCES.includes(sourceUrl.hostname) &&
			...
			const date =
				validItem && validItem.properties['dt-published']
					? new Date(validItem.properties['dt-published'][0].toString())
					: new Date();
			const foundTypes = Object.values(VALID_WEBMENTION_TARGET_TYPE).filter((v) =>
				Object.keys(validItem?.properties || {}).includes(v)
			);
			const { success, error: dbErr } = await db
				.prepare('insert into mentions(slug, url, date, type, mfItem) values (?, ?, ?, ?, ?);')
				.bind(
					resolvedSlug,
					sourceUrl.toString(),
					date.toISOString(),
				...
				if (customCache && platform.context?.waitUntil) {
					...
					platform.context.waitUntil(
						customCache.put(
							resolvedUrl,
							new Response('', {
								headers: { 'Cache-Control': 'max-age=0', Expires: new Date(0).toUTCString() }
							})
						)
					);
				}
